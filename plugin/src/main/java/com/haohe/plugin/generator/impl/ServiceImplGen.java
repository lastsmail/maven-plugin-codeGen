 ...
 *  ServiceImpl 生成器
 */
public class ServiceImplGen extends AbsGenerator {
    ...
    public void generator(ClassModel clsModel) {
        ...
        try {
            setTemplate(exampleCfg.getTemplate("serviceImpl.ftl"));
            ...
            logger.error("ServiceImpl文件未找到",e);
        } catch (TemplateException e) {
            logger.error("ServiceImpl模板未找到",e);
        }
    }
    ...
    protected String createFileName(ClassModel clsModel) {
        ...
        if(getUseModelPackage()){
            ...
            FileDirUtil.createFileDir(sb.toString()+ FileType.SERVICEIMPL.getPackageName());
        }
        sb.append(FileType.SERVICEIMPL.getPackageName());
        sb.append(clsModel.getClsName());
        sb.append(FileType.SERVICEIMPL.getStamp());
        logger.info("ServiceImpl文件创建名字:{}",sb.toString());
        return sb.toString();
    }
}
